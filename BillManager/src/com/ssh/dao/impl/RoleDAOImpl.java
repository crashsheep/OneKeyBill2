 ...
 */
@Repository
public class RoleDAOImpl extends MyHibernateDaoSupport {
	...
	public Role findById(java.lang.Integer id) {
		...
		try {
			Role instance = (Role) getSession().get("com.Role", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Role> findByExample(Role instance) {
		...
		try {
			List<Role> results = (List<Role>) getSession()
					.createCriteria("com.Role").add(create(instance)).list();
	...
	public List<Role> findByAuthId(Object authId) {
